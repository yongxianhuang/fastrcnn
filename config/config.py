...
# config file dir
__C.CONFIG_FILE = osp.abspath(osp.join(osp.dirname(__file__)))
...
def get_output_dir(imdb, net=None):
    ...
    (if not None).
    """
    path = osp.abspath(osp.join(__C.ROOT_DIR, 'output', __C.EXP_DIR, imdb))
    if net is None:
        return path
    else:
...
# ---------------------------------------------------------------------------- #
__C.SOLVER = edict()
__C.SOLVER.OPTIMIZER_NAME = "SGD"
__C.SOLVER.MAX_EPOCHS = 50
...
__C.SOLVER.CHECKPOINT_PERIOD = 10
__C.SOLVER.LOG_PERIOD = 100
